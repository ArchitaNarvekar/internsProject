    ...
    >
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="Sub" :props="props">{{ props.row.Sub }}</q-td>
          <q-td key="Subj" :props="props">{{ props.row.Subj }}</q-td>
    ...
    <div class="lt-md" >
      ...
      <div>
        <q-card class="q-mb-sm" v-for="(task,index) in data" :key="index">
          ...
          <q-card-section>
            <div class="text-h6">{{task.Sub}}</div>
          </q-card-section>
          <q-card-section>
            <div class="text-h6">{{task.Subj}}</div>
          </q-card-section>
          <q-separator />
          <q-card-section>
  ...
  export default {
    ...
    data () {
      return {
        ...
        { name: 'Subject', label: 'CreatedOn',align: 'left', field: 'Subject' },
        { name: 'Sub', label: 'FixedOn',align: 'left', field: 'Sub' },
        { name: 'Subj', label: 'VerifiedOn',align: 'left', field: 'Subj' },
      ...
      fetchCommunications() {
        ...
        if (responseData.Status === "OK"){
          ...
          this.fetchedCommunications.forEach(function(communication){
            var obj = {}
            obj["CommunicationId"]=communication.EntityId
            obj["Subject"]=self.truncateString(communication.Detail.CreatedOn)
            obj["Sub"]=self.truncateString(communication.Detail.FixedOn)
            obj["Subj"]=self.truncateString(communication.Detail.VerifiedOn)
