...
<template>
  <div>
    <h7 class="chart-title">DIFFERENCE OF VERIFIED-ON W.R.T. CREATED-ON</h7>
    <br>
  </div>
  ...
  <input type="date" id="start-date" v-model="startDate" @change="drawChart" />
  <label for="end-date">End Date:</label>
  <input type="date" id="end-date" v-model="endDate" @change="drawChart" />
</div> -->
  ...
  methods: {
    drawChart() {
      ...
      const y = d3.scaleLinear()
        .domain([0, d3.max(timeKeys, key => Math.max(avgCounts[key].created, avgCounts[key].verified))])
        .nice()
        .range([height, 0]);

      const color = d3.scaleOrdinal()
        .domain(['created', 'verified'])
        .range(['green', 'purple']);
      ...
      svg.selectAll('.bar-group')
        ...
        .selectAll('.bar')
        .data(d => ['created', 'verified'].map(key => ({ key, value: avgCounts[d][key] || 0 })))
        .enter().append('rect')
        .attr('class', 'bar')
        .attr('x', d => x.bandwidth() / 2 * (d.key === 'verified' ? 1 : 0))
        .attr('y', d => y(d.value))
    ...
    calculateAverageCounts(counts) {
      ...
      Object.keys(counts).forEach(timeKey => {
        avgCounts[timeKey] = {
          ...
          verified: counts[timeKey].total - counts[timeKey].verified,
        };
      });
      return avgCounts;
    },
